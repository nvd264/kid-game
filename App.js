...
const GardenHarvestGame = ({ playSound, onExit, fontsLoaded, toggleMusic, musicEnabled }) => {
  ...
  /** Window origin of the grid hit view; all plotLayoutsRef entries derived from this (no per-cell measureInWindow). */
  const gardenGridHitRef = useRef(null);
  const gardenGridLayoutRef = useRef(null);
  /** True while finger drag-paints on the field — skip heavy feedback/sounds for performance. */
  ...
  /** Line combo signatures already rewarded (format r:row:ids or c:col:ids). */
  ...
  const applyAllComboUnlocks = useCallback((plotsIn) => {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      out = out.map((p) => {
        ...
        return { ...p, unlocked: true, previewCrop: p.previewCrop ?? randomGardenCrop() };
      });
      changed = true;
    }
  ...
  const initPlotAnim = useCallback((id) => {
    if (plotAnimsRef.current[id]) {
      ...
      if (!plotAnimsRef.current[id].unlockPulse) {
        plotAnimsRef.current[id].unlockPulse = new Animated.Value(0);
      }
      return;
    }
    plotAnimsRef.current[id] = {
      ...
      unlockPulse: new Animated.Value(0),
      timers: [],
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (allUnlocked && allPlayableEmpty) {
  ...
  const pickPlotUnderFinger = useCallback((pageX, pageY, validState) => {
    ...
    const lx = pageX - g.x;
    const ly = pageY - g.y;
    return pickPlotFromLocalXY(lx, ly, validState);
  }, [pickPlotFromLocalXY]);
  ...
  const flushFieldDragPendingMove = useCallback(() => {
    ...
    if (p) {
      fieldDragPendingRef.current = null;
      handlersRef.current.applyDragAtLocal?.(p.x, p.y);
    }
  }, []);
                ...
                >
                  ...
                  <View style={styles.gardenLockedStoneIconWrap}>
                    <MaterialCommunityIcons
                      ...
                      size={Math.round(plotSize * 0.34)}
                      color="rgba(32,34,38,0.9)"
                    />
                  </View>
                </LinearGradient>
              </View>
      ...
      <Modal visible={gardenWinVisible} transparent animationType="fade">
        <View style={styles.gardenWinBackdrop}>
          <LinearGradient colors={[FARM.cardFront, FARM.cardMatched]} style={styles.gardenWinCard}>
            ...
            <Text style={[styles.gardenWinSub, { fontFamily: F8 }]}>
              Bạn đã mở hết ruộng và thu hoạch xong. Tuyệt vời!
            </Text>
            <AnimatedPressable onPress={() => { playSound('tap'); onExit(); }}>
...
const styles = StyleSheet.create({
  ...
  gardenLockedStoneBlob: {
    width: '88%',
    height: '88%',
    borderRadius: 999,
    borderWidth: 2,
    borderColor: 'rgba(48,50,54,0.45)',
    overflow: 'hidden',
    justifyContent: 'center',
    alignItems: 'center',
