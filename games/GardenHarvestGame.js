...
const GardenHarvestGame = ({ playSound, onExit, fontsLoaded, toggleMusic, musicEnabled }) => {
  ...
  const getRipeCropArt = useCallback((plot) => (plot.state === 'ripe' ? plot.crop?.ripeArt : null), []);
      ...
      <View style={styles.gardenRootPortrait}>
        ...
        <View style={styles.gardenPortraitColumn}>
          <GardenGrid
            ...
            gardenGridHitRef={gardenGridHitRef}
            gardenGridLayoutRef={gardenGridLayoutRef}
            gardenFieldDragActiveRef={gardenFieldDragActiveRef}
            gardenDragFromDockRef={gardenDragFromDockRef}
            lastFieldPlotDragRef={lastFieldPlotDragRef}
            fieldDragRafRef={fieldDragRafRef}
            fieldDragPendingRef={fieldDragPendingRef}
            hoveredPlotId={hoveredPlotId}
            GARDEN_GRID_COLS={GARDEN_GRID_COLS}
            GARDEN_GRID_ROWS={GARDEN_GRID_ROWS}
            GARDEN_GRID_TOTAL={GARDEN_GRID_TOTAL}
          ...
          <GardenDock
            ...
            handlersRef={handlersRef}
            basketRef={basketRef}
            basketLayoutRef={basketLayoutRef}
            basketBounce={basketBounce}
            dragFloatX={dragFloatX}
            dragFloatY={dragFloatY}
            dragFloatScale={dragFloatScale}
            dragFromFabRef={dragFromFabRef}
            gardenDragFromDockRef={gardenDragFromDockRef}
            GARDEN_TOOLS={GARDEN_TOOLS}
            pickPlotUnderFinger={pickPlotUnderFinger}
            applyDockToolToPlot={applyDockToolToPlot}
            gardenFieldDragActiveRef={gardenFieldDragActiveRef}
            lastDockPlotRef={lastDockPlotRef}
            selectedToolIdRef={selectedToolIdRef}
            setDragTool={setDragTool}
            setHoveredPlotId={setHoveredPlotId}
            hoveredPlotIdRef={hoveredPlotIdRef}
            syncGardenPlotLayoutsFromGrid={syncGardenPlotLayoutsFromGrid}
          />
        </View>
        {gardenFlyAndDrag}
